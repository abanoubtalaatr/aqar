...
class Ad extends Model
{
    ...
    public function similar_productse()
    {
        return $this->hasMany(Ad::class, 'category_id', 'category_id')
            ->where('id', '!=', $this->id)
            ->whereBetween('area', [$this->area * 0.9, $this->area * 1.1])
            ->whereBetween('price', [$this->price * 0.9, $this->price * 1.1])
            ->limit(3);
    }
    ...
    public function similar_products()
    {
        return $this->hasMany(Ad::class, 'category_id', 'category_id')
            ->where('id', '!=', $this->id)
            ->whereBetween('area', [round($this->area * 3, 2), round($this->area * 3, 2)]) // Round to avoid precision issues
            ->whereBetween('price', [round($this->price * 3, 2), round($this->price * 3, 2)])
            ->limit(3);
    }
    // ============================End Relation===================================
